...
function resolveAssetPath(path) {
    ...
    const resumeRootIndex = pathParts.lastIndexOf("resume_algo");

    if (resumeRootIndex < 0) {
        return value;
    }

    const levelsToRepoRoot = pathParts.length - resumeRootIndex;
    return `${"../".repeat(levelsToRepoRoot)}${value}`;
}
    ...
    return `
        <div class="media-preview-grid media-preview-grid-${variant}">
            ${visible.map((item, index) => {
                ...
                const mediaPath = resolveAssetPath(item.path);
                const inner = item.kind === "image"
                    ? `<img src="${escapeHtml(mediaPath)}" alt="${escapeHtml(item.caption || "Project image")}">`
                    : `<video muted playsinline preload="metadata">
                            <source src="${escapeHtml(mediaPath)}" type="${item.path.toLowerCase().endsWith(".mov") ? "video/quicktime" : "video/mp4"}">
                       </video>
                       <span class="video-tag">Video</span>`;

                return `
                    <a class="media-thumb ${item.kind === "video" ? "video" : ""}" href="${escapeHtml(mediaPath)}" target="_blank" rel="noopener noreferrer">
                        ${inner}
                        ${overlay}
                    </a>
                `;
...
function renderSidebar(data) {
    const avatarPath = resolveAssetPath(data.profile.avatar);

    return `
        <section class="profile-card" data-reveal>
            <div class="profile-top">
                <div class="avatar-shell">
                    <img src="${escapeHtml(avatarPath)}" alt="${escapeHtml(data.profile.name)}">
                </div>
